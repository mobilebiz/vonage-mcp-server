...
ただし**SMS の課金はセグメント単位**です（[SMS の課金単位](#sms-の課金単位--セグメント)）。件数の枠は「操作の回数」を抑えるもので、費用そのものを抑えたい場合は `segments` の枠を使ってください。
...
| 環境変数 | 必須 | 説明 |
| --- | --- | --- |
| `OAUTH_ISSUER` | ✅ | 認可サーバーの issuer。トークンの `iss` と照合します。https のみ（ループバックのみ http 可）。 |
...
- **`GET /.well-known/oauth-protected-resource`**（およびリソースのパスを付けた `/.well-known/oauth-protected-resource/mcp`）— RFC 9728 の保護リソースメタデータ。**認証不要**です。クライアントはトークンを持つ前にここを読み、どの認可サーバーへ行けばよいかを知ります
...
>
> ID トークンの `aud` は**クライアント識別子**です。**既定（`OAUTH_AUDIENCE` を設定しない）なら、期待する `aud` はこのサーバーがメタデータで配っている URI なので、その URI を IdP 側でクライアント識別子として登録しない限り、ID トークンとは一致しません。** 追加の設定は要りません（登録してしまった場合に何が起きるか、サーバーが何を検査するかは後述します）。
>
...
>
> **トークンが `typ: at+jwt` を持っていれば、要求していなくてもこの2つの検査は行いません。** 「要求していない」ことと「実際に無い」ことは別で、標識を持っているトークンはそれ自体がアクセストークンの証明だからです（`typ` は署名の対象です）。
>
> **あわせて、トークンの `azp` / `client_id` がこのサーバーの audience と同じなら拒否します。** その状態は「この URI がクライアント識別子として登録されている」ことそのもので、上の根拠が崩れているためです。ただし**どちらの claim も必須ではないので、これで塞ぎきれるわけではありません。** 確実にしたい場合は `OAUTH_REQUIRE_AT_JWT=true` か `OAUTH_REQUIRED_SCOPE` を設定してください。
>
> `at_hash` / `c_hash` を持つトークンは常に拒否しますが、**これは当てにできません** — どちらも条件付きの claim で、認可コードフローの ID トークンには通常入っていないためです。「無いこと」は根拠になりません。

> [!NOTE]
> **当初はこの標識を無条件に要求していました。** しかし **MCP 仕様に最も沿っている WorkOS ですら `typ: at+jwt` を付けない**ため、素直に設定した利用者が全員 401 を踏む状態でした。**その URI をクライアント識別子として登録しない限り衝突しない構成にまで、標識を求めていたのが誤り**で、条件を実態に合わせています（登録してしまった場合の扱いは上記のとおりです）。
