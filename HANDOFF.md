# セッション引き継ぎ — 2026-09-09 (更新4: v3.1.1 リリース完了)
...
**v3.1.0 を main にマージ済み。未マージの PR はありません。**

| | |
|---|---|
| `main` | **v3.1.1**（`0300f7e`）。OAuth 2.1（PR #4）/ 依存の脆弱性（#5, #7）/ **名乗る版の修正（#6）** |
| 未マージの PR | **なし** |
| テスト | **535 passed** |
| 本番依存の脆弱性 | **0 件**（`fast-uri` / `qs` を #5、`hono` を #7 で解消） |
| GitHub Release (Latest) | **v3.1.1**（2026-09-09）。**ダウンロードして5か所すべてのバージョンを確認済み** |
| Cloud Run | **リビジョン `00047-rn7` = v3.1.1**（2026-09-20、`ALLOWED_NUMBERS` 追加の環境変数更新）。`/health` が `3.1.1` を返すことを確認済み。**対応不要** |

**実機検証 — README の対応プラットフォーム表で ✅ は5行。残るは n8n だけです。**
...
| n8n | ⬜ 未実施 |
| ChatGPT（カスタムプラグイン / コネクタ） | 🔨 **OAuth リソースサーバーモードを実装しました**（2026-09-08、ブランチ `feat/oauth-resource-server`）。**実機では未検証。** → 4.3 |
...
### 4.3 OAuth リソースサーバーモード — **v3.1.0 でマージ済み。実機は未検証**（2026-09-08）
...
- **ID トークンをアクセストークンとして受け取れた。** 対策を「`at_hash`/`c_hash` が無いこと」に置いていたが、**あの2つは条件付きの claim で、認可コードフローの ID トークンには入っていない。**「無いこと」は根拠にならない。`OAUTH_REQUIRE_AT_JWT`（既定 true）か `OAUTH_REQUIRED_SCOPE` のどちらかを必須にし、**「アクセストークンであることを積極的に示すもの」**を要求する形に変えた
- **ループバック http の許容が全インターフェースに漏れていた。** OAuth を「認証済み」と数えたため `0.0.0.0` に bind され、平文でトークンを受け取るサーバーが外に出ていた

1. **実機検証。** ChatGPT のプラグイン画面から実際に繋いだ確認はまだです。README にもその旨を明記しています
2. **IdP の選定。** 繋がるかどうかは **IdP のクライアント登録方式**（Client ID Metadata Documents か DCR）で決まります。ChatGPT や Claude は IdP と事前の関係を持たないためです。**ここが実質的な残作業**
3. **D-11 の再判断**（VONAGE_MCP-1）と、VONAGE_MCP-2 の §3.3 / 4.4 への反映。**実装記録は VONAGE_MCP-33 に作成済み**
4. **タグと GitHub Release**（→ 4.5）
