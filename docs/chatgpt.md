...
**必要なのはこの3つだけです（v3.2.0 以降）。** `OAUTH_AUDIENCE` を上書きしなければ、他の設定は要りません。

> [!IMPORTANT]
> **2026-09-11 に実機で通したのは、この3つだけの構成ではありません。** 当時動いていたのは v3.1.1 で、**標識（`typ: at+jwt` か必須 scope）が無条件に必要**でした。WorkOS は `typ` を付けないため、実際には次の2つを足しています。
> 
> ```sh
> OAUTH_REQUIRE_AT_JWT=false
> OAUTH_REQUIRED_SCOPE=email
> ```
> 
> **v3.2.0 で「3つだけ」で動くようにしたのがこの変更の中身ですが、その3つだけの構成自体は、まだ ChatGPT から実機で確認していません。**上の5つ（＝実機で通した構成）はそのまま使えます。
...
**この要求は条件付きに改めました。** ID トークンの `aud` はクライアント識別子なので、`OAUTH_AUDIENCE` を上書きしていなければ（＝期待する `aud` がこのサーバーの URI のままなら）、**その URI をクライアント識別子として登録しない限り衝突しません**。**既定で**標識を要求するのは**上書きしたときだけ**です。なお `OAUTH_REQUIRE_AT_JWT=true` を明示すれば、上書きが無くても `typ: at+jwt` を要求できます（厳しくする道は常に開いています）。

> [!WARNING]
> **「登録しない限り」が条件です。** `OAUTH_RESOURCE` の URI を IdP 側でクライアント識別子として登録すると、`OAUTH_AUDIENCE` を上書きしていなくても ID トークンの `aud` が一致します。サーバーは次の2つでこれを防ぎます。
>
> - `aud` がこのサーバーの URI **単独**であることを要求する（複数 audience のトークンは拒否）
> - トークンの `azp` / `client_id` が audience と一致したら拒否する
>
> **ただし `azp` も `client_id` も必須の claim ではないため、どちらも付けない IdP では検出できません。** アクセストークンであることを確実に見分けたい運用では、`OAUTH_REQUIRE_AT_JWT=true` か `OAUTH_REQUIRED_SCOPE` を設定してください。**IdP 側でこの URI をクライアント識別子として登録しないこと**が前提です。

**そのため、v3.2.0 の WorkOS では追加の設定が要りません。** 必須の3つだけで動きます（ただし上記のとおり、**その構成は実機未確認**です。実機で通っているのは標識を足した5つの構成です）。
