...
## 5. 実機で確認できたこと（2026-08-25 / 2026-09-21 に再確認）

> [!NOTE]
> **2026-09-21 に、サーバー v3.2.0 で再確認しました。** Agent Engine を直接叩き
> （`:streamQuery`）、`send_sms` の `function_call` → `function_response`（`dry_run_success`）まで
> 確認しています。**引数名は `message` で、このサーバーのスキーマと一致していました。**
>
> **Apps の画面からも同日に再確認しました。** `@` で指名 → `dry_run` の提示 → 承認ウィンドウ → 実送信 →
> `get_sms_status` が `delivered`。**読み取り専用ツールは承認ウィンドウ無しで即実行**されました。
>
> **承認ウィンドウは `Action Name` / `To` / `Message` を編集可能なフォームで見せます。** ここに出る値が
> 実際にツールへ渡る値なので、**承認の根拠にしてよいのはこちら**です（`tool_code` ではなく）。
...
**2026-09-21 には、`tool_code` を書いただけでツールを呼ばない例も出ました。** 「dry_run で検証だけして、
実際には送信しないでください」と頼んだところ、エージェントは `tool_code` のテキスト
（`send_sms(to=..., from_='VonageMCP', text='...', dry_run=True)`）を返しただけで、**`function_call` は
1件も発生しませんでした。** 「`send_sms` を `dry_run=true` で**実際に呼び出してください**」と書き直したら
呼びました。**「呼ぶな」と誤解されうる書き方を避けてください。**
...
> **画面のどこを見るかで話が変わります。**
>
> - **`tool_code` の表示は当てになりません**（上の表のとおり、実際の呼び出しと食い違った実測が2件）
> - **承認ウィンドウは当てになります。** `Action Name` / `To` / `Message` を編集可能なフォームで見せ、
>   **その値がそのまま実送信の呼び出しに渡ります**（2026-09-21 に確認）。**承認する前にここを読んでください**
>
> ただし承認ウィンドウが出るのは**破壊的ツールだけ**で、dry_run の呼び出しには出ません。
> **「dry_run で見た宛先」と「実際に送られる宛先」が同じである保証は、依然としてありません。**
>
> **したがって、宛先を確実に縛れるのはサーバー側の `ALLOWED_NUMBERS` だけです。**
> 検証中は自分の番号だけを許可してください。

この制約は、設計時に一度検討したうえで受け入れているものです。
