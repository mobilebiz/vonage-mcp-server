...
- send_sms の本文は最大 160 文字。これを超える内容は、送信前に必ず要約すること。
  勝手に分割して複数回 send_sms を呼び出してはならない。
- bulk_sms_from_csv の message 列も 1 行あたり 160 文字まで。超えた行は送信されずスキップされ、
  レスポンスの too_long_rows に件数が入る。CSV を作る前に各行を要約すること。
- 日本語（非 ASCII）を含む本文は 70 文字を超えると複数通に分割され課金が増える。
  可能な限り 70 文字以内に収め、超える場合はその旨をユーザーに伝えること。
- 送信元表示名（from）は英数字 3〜11 文字（先頭は英字、例: VonageMCP）か、
  E.164 形式の電話番号。数字のみの短い文字列や日本語は使用不可。dry_run の時点で検証される。
...
- generate_jwt が返すトークンは機密情報である。ユーザーが明示的に要求した場合のみ実行し、
  ログや要約に貼り付けないこと。
...
`to` に正規化後の番号が入るため、**「AI が意図した宛先」と「実際に送られる宛先」が
一致しているかをユーザーが目視確認できる**のが dry_run の最大の価値です。
...
サーバーは 1 時間あたりの**送信・架電件数**を、**ツールごとに独立して**制限します
...
実際の運用量に合わせて調整してください（`0` を指定すると無効になります）。
